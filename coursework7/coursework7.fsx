...
let maxInList (list: int list) = 
   let rec worker acc list =
    match list with 
    | [] -> acc
    | head::tail -> if acc > head then worker acc tail else worker head tail
   worker 0 list 
                   

maxInList [1..10000000]
(*  
  Task 2:
...
let reverse list = 
   let rec worker acc list =
    match list with
    | [] -> acc
    | head::tail -> worker (List.append [head] acc) tail
   worker [] list 
...
let testTree = Branch(Branch(Leaf 9,Leaf 15),Branch(Leaf 345,Leaf 1488))
maxInTree testTree
...
let maxInTreeAcc tree =
    let rec worker acc tree cont=
        match tree with
        | Leaf pew-> cont(if acc > pew then acc else pew)
        | Branch(left,right) -> worker acc left (fun acc -> worker acc right cont)
    worker 0 tree (fun f -> f)

maxInTreeAcc testTree
(*
  Task 5:
...
let squares = streamMap (fun n -> n * n) (from 1)
streamTail squares
...
let oddNumbers = streamFilter (fun x -> x % 2 = 0) (from 5)

streamTail oddNumbers        